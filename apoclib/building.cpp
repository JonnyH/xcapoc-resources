...
Building::Building(Organisation &owner,
	std::string name,
	Rect2d<unsigned short> position)
	: owner(owner), name(name), position(position)
{
}
...
Building*
Building::loadFromFile(ALLEGRO_FILE *file,
	...
	std::vector<std::string> &names)
{
	...
	std::cout << "Loaded building:\n"
		...
		<< "Unknown2 = 0x" << std::hex << bld.unknown2 << std::dec << " (Building type?)\n"
		<< "\t\"" << building_types[bld.unknown2] << "\"\n"
	...
	return new Building(owner, name, position);
}
